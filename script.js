...
const wordsPerMinute = function () {
  ...
  const wpmm =
    totalLetterTyped.length / (timeInminutes(selectedTime.value) * 5);
...
const getFeedBack = function (wpm) {
  ...
  else if (wpm >= 50 && wpm < 60) return feedback.moderate_performance;
  else if (wpm >= 30 && wpm < 50) return feedback.average;
  else if (wpm >= 20 && wpm < 30) return feedback.poor_performance;
  else if (wpm < 20 && wpm >= 0) return `you are noob ! keep up the Hard work`;
};
...
const updateQuote = function (time) {
  if (time === 15) return fifteenSecondQuotes[generateRandomNumber()];
  else if (time === 30) return thirtySecondQuotes[generateRandomNumber()];
  else if (time === 60) return sixtySecondQuotes[generateRandomNumber()];
};
...
selectedTime.addEventListener("change", function () {
  ...
  if (pickedTime === 15) updateTheQuoteandReset(pickedTime);
  else if (pickedTime === 30) updateTheQuoteandReset(pickedTime);
  else if (pickedTime === 60) updateTheQuoteandReset(pickedTime);
});
// start Action
...
const startAction = function () {
  if (isIntervalRunning === true) {
    ...
    time = booltimeCheck
      ? remainingTime
      : Number(document.getElementById("timer").value);
